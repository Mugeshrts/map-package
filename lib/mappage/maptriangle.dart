...
class Maptriangle extends StatefulWidget {
  const Maptriangle({super.key});

  @override
  State<Maptriangle> createState() => _MappageState();
}

class _MappageState extends State<Maptriangle> {
  ...
  final double totalHeight = baseIcon.height + 90; // Extra space for badge
  final double triangleHeight = 50; // 🔺 Height of the triangle
  final double triangleBase = 80;   // 🔺 Base width of the triangle

  final ui.PictureRecorder recorder = ui.PictureRecorder();
  final Canvas canvas = Canvas(recorder);
  final Paint paint = Paint();

  // Draw base icon below the triangle
  canvas.drawImage(baseIcon, Offset(0, triangleHeight + 10), paint);

  // 🎨 **Draw Triangle Shape**
  final Paint trianglePaint = Paint()..color = Colors.red;
  final Path trianglePath = Path()
    ..moveTo((iconWidth - triangleBase) / 2, 10) // 🔺 Top center
    ..lineTo((iconWidth + triangleBase) / 2, 10) // 🔺 Bottom right
    ..lineTo(iconWidth / 2, triangleHeight + 10) // 🔺 Bottom center (point)
    ..close();

  canvas.drawPath(trianglePath, trianglePaint);

  // 🏷 **Draw Centered Text Inside Triangle**
  final TextPainter badgeText = TextPainter(
    text: TextSpan(
      text: value,
      style: const TextStyle(color: Colors.white, fontSize: 30, fontWeight: FontWeight.bold), // 🔹 Adjust font size
    ),
  ...
  badgeText.paint(
    canvas,
    Offset((iconWidth - badgeText.width) / 2, triangleHeight / 3), // Center text inside the triangle
  );
